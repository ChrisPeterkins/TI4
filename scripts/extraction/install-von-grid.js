...
/**
 * Install and adapt von-grid hexagonal math library
 */
...
async function extract() {
  try {
    console.log('  🎲 Installing von-grid hexagonal math...');
    
    // Create geometry directory
    await fs.mkdir(outputPath, { recursive: true });
    
    // Create hexagonal grid utilities based on von-grid
    const hexGridCode = `/**
...
`;
    ...
    await fs.writeFile(
      path.join(outputPath, 'HexGrid.ts'),
      hexGridCode
    );
    
    console.log('    ✅ Hexagonal grid system installed');
    
  } catch (error) {
    console.error('  ❌ von-grid installation failed:', error);
    throw error;
  }
}
